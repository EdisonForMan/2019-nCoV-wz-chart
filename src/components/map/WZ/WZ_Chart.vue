...
import { DATA_WENZHOU, GEO_WENZHOU } from "../mapJson";

import { qzrs, zyrs, gklzb } from "../data/mapData";
...
export default {
  ...
  mounted() {
    ...
    that.convertData = qzrs
      .filter(item => item["区县"] != "总计")
      .map(item => {
        return {
          itemStyle: {},
          name: item["区县"],
          value: that.GEO_WENZHOU[item["区县"]].concat(item["确诊统计数"])
        };
      });
    ...
    that.zyrs.map(item => {
      zyObj[item["所属区县"]] = item["治愈数量"];
    });

    that.convertData.map(item => {
      item.value = item.value.concat(zyObj[item.name] || "0");
      return item;
    });
    ...
    that.gklzb.map(item => {
      gklObj[item["区县"]] = item["值"];
    });

    that.convertData.map(item => {
      item.value = item.value.concat(gklObj[item.name] || "0");
      return item;
    });

    // console.log(that.convertData);
